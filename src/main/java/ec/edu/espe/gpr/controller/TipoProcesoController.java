...
public class TipoProcesoController {
    ...
    @GetMapping(path = "/obtenerProcesoPorNombreProceso/{nombreProceso}")
    public ResponseEntity<TipoProceso> obtenerProcesoPorNombreProceso(@PathVariable String nombreProceso) {
        try {
            TipoProceso tipoProceso = this.tipoProcesoService.obtenerProcesoPorNombreProceso(nombreProceso);
